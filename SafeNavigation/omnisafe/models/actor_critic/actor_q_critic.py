...
from omnisafe.models.critic.critic_builder import MOCriticBuilder, CriticBuilder
...
class ActorQCritic(nn.Module):
    ...
    def step_raw(self, obs):
        with torch.no_grad():
            return self.actor.get_raw_dist(obs)
...
class MOActorQCritic(nn.Module):
    """Class for ActorQCritic.
    ...
    def __init__(
        ...
        self.critic: Critic = MOCriticBuilder(
            obs_space=obs_space,
            act_space=act_space,
            hidden_sizes=model_cfgs.critic.hidden_sizes,
            activation=model_cfgs.critic.activation,
        ...
        self.target_critic: Critic = deepcopy(self.critic)
        ...
        if model_cfgs.actor.lr is not None:
            ...
            self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=model_cfgs.actor.lr)
        if model_cfgs.critic.lr is not None:
            self.critic_optimizer: optim.Optimizer
