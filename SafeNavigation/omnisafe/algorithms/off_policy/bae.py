...
class BAE(SAC):
    ...
    def qr_loss(self,
        current_quantiles_in: torch.Tensor,  # (B, nq, nc)
        target_quantiles: torch.Tensor,      # (B, nq, nc)
    ) -> torch.Tensor:
        
        B, nq, nc = current_quantiles_in.shape
        taus = (torch.arange(nq, dtype=current_quantiles_in.dtype, device=current_quantiles_in.device) + 0.5) / nq

        delta = target_quantiles[:, :, None, :] - current_quantiles_in[:, None, :, :]
        abs_delta = torch.abs(delta)
        huber = torch.where(
            abs_delta <= 1,
            0.5 * torch.square(delta),
            abs_delta - 0.5
        )
        tau = taus[None, None, :, None]
        weight = torch.abs(tau - (delta < 0).float())
    ...
    def _update_reward_critic(
        ...
        with torch.no_grad():
            ...
            # next_q_r = torch.stack(next_q, -1)
            # next_q_r = next_q_r[...,:-int(self.tail_reward)]
        ...
        # loss = self.qr_loss(current_q, target_q_value_r)
        loss = loss.mean()
    ...
    def _update_cost_critic(
        ...
        with torch.no_grad():
            ...
            # next_quantiles_cost = next_quantiles_cost[...,int(self.tail_cost):]
        ...
        # loss_c = self.qr_loss(current_q_c, target_q_value_c)
        loss_c = loss_c.mean()
    ...
    def _update_actor(  # pylint: disable=too-many-arguments
        ...
        if self._epoch > self._cfgs.algo_cfgs.warmup_epochs:
            ...
            explore_cost = self._env.onpolicy_cost.mean()*40
            self._lagrange.update_delta_fly(explore_cost)
            # self._lagrange.update_auto_truncation(explore_cost)
    ...
    def _loss_pi(
        ...
        with torch.no_grad():
            ...
            q_current_cost = q_current_cost_raw[:,16:].mean(-1).mean(-1)
        ...
        q_cost = torch.stack(q_cost, -1)[:,16:]
