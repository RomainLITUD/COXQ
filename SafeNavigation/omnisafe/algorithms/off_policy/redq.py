...
class REDQ(SAC):
    ...
    def _update_reward_critic(
        ...
        with torch.no_grad():
            ...
            # print(next_q_r.shape)
            rand_idx = torch.randperm(next_q_r.shape[-1])[:2]
            next_q_value_r = torch.amin(next_q_r[:, rand_idx], -1) - next_logp * self._alpha
        ...
        loss = (current_q - target_q_value_r[:, None])**2
        loss = loss.mean()
    ...
    def _update_cost_critic(
        ...
        with torch.no_grad():
            ...
            next_q_cost = torch.stack(next_q_cost, -1)
            rand_idx = torch.randperm(next_q_cost.shape[-1])[:2]
            target_q_value_c = cost + self._cfgs.algo_cfgs.gamma * (1 - done) * torch.amin(next_q_cost[:, rand_idx], -1)
        q_value_c = self._actor_critic.cost_critic(obs, action)
        current_q_c = torch.stack(q_value_c, -1)

        loss_c = (current_q_c - target_q_value_c[:, None])**2
        loss_c = loss_c.mean()
    ...
    def _update_actor(  # pylint: disable=too-many-arguments
        ...
        loss_pi, cost_pred = self._loss_pi(obs, act)
        ...
        if self._epoch > self._cfgs.algo_cfgs.warmup_epochs:
            ...
            #self._lagrange.update_auto_truncation_reward(reward_test/10., reward_pred.detach())
        self._logger.store(
            {
        ...
        # self._lagrange.update_delta_fly(Jc)
    ...
    def _loss_pi(
        ...
        q_reward = torch.stack(q_reward, -1).mean(-1)

        with torch.no_grad():
            q_current_cost = self._actor_critic.cost_critic(obs, act)
            q_current_cost = -torch.stack(q_current_cost, -1).mean(-1)
            #q_current_reward = self._actor_critic.reward_critic(obs, act)
            #q_current_reward = torch.concatenate(q_current_reward, -1).mean(-1)
        lam = self._lagrange.lagrangian_multiplier.item()
        q_cost = self._actor_critic.cost_critic(obs, action)
        q_cost = -torch.stack(q_cost, -1).mean(-1)
        if self._epoch > self._cfgs.algo_cfgs.warmup_epochs:
            ...
            actor_loss = self._alpha * log_prob - q_reward + lam*q_cost
        return actor_loss.mean(), q_current_cost.mean() #, q_current_reward.mean()
