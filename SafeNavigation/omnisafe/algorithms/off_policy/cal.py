...
class CAL(SAC):
        ...
        for epoch in range(self._epochs):
            ...
            for sample_step in range(
                ...
                self._env.rollout(
                    ...
                    lam = self._lagrange.lagrangian_multiplier.exp().clone().detach(),
                    cost_limit = 2.5,
                    delta = 3.,
    ...
    def _update(self) -> None:
        for _ in range(self._cfgs.algo_cfgs.update_iters):
            ...
            if self._update_count % self._cfgs.algo_cfgs.target_update_frequency == 0:
    ...
    def _update_cost_critic(
        ...
        with torch.no_grad():
            ...
            # rand_idx = torch.randperm(next_q_cost.shape[-1])[:4]
            # next_q_cost = torch.amax(next_q_cost[:, rand_idx], -1)

            next_q_cost = next_q_cost.mean(-1) + 0.5 * next_q_cost.std(-1)
        ...
        loss_c = torch.square(current_q_c - target_q_value_c[..., None]).mean()
    ...
    def _loss_pi(
        ...
        with torch.no_grad():
            ...
            q_current_cost = torch.stack(q_current_cost, -1)
            q_current_cost = q_current_cost.mean(-1) + 0.5 * q_current_cost.std(-1)

        q_cost = self._actor_critic.cost_critic(obs, action)
        q_cost = torch.stack(q_cost, -1)
        q_cost = q_cost.mean(-1) + 0.5 * q_cost.std(-1)
        ...
        return actor_loss.mean(), q_current_cost.mean()
