...
class SACDtqc(SAC):
    ...
    def quantile_huber_loss(self, current_quantiles_in,  # (B, nq, nc)
        ...
        target_quantiles = target_quantiles.swapaxes(-1, -2)
        n_quantiles = current_quantiles.shape[-1]

        cum_prob = (torch.arange(n_quantiles, dtype=current_quantiles.dtype,
                                device=current_quantiles.device) + 0.5) / n_quantiles

        pairwise_delta = target_quantiles[:, :, None, :] - current_quantiles[:, :, :, None] #(B, nc, nq, nqt)
        ...
        # quantile weighting
        loss = (cum_prob[None, None, :, None] - indicator).abs() * huber_loss
    ...
    def _update_reward_critic(
        ...
        with torch.no_grad():
            ...
            next_q_r = torch.stack(next_q, -1) # (B, nq*nc)

            # next_q_r = next_q_r[...,:-int(self.tail_reward*Nr)]
            next_q_value_r = next_q_r - next_logp[:, None, None] * self._alpha
            target_q_value_r = reward[:, None, None] + self._cfgs.algo_cfgs.gamma * (1 - done[:,None, None]) * next_q_value_r
    ...
    def _update_cost_critic(
        ...
        with torch.no_grad():
            ...
            next_quantiles_cost = torch.stack(next_q_cost, -1)
            #if self._epoch > self._cfgs.algo_cfgs.warmup_epochs:
            # trunc = self._lagrange.auto_truncation.item()
            # kk = int(trunc) if trunc > 0 else int(trunc) -1
            # next_quantiles_cost = next_quantiles_cost[..., -min(kk, 0): 125 - max(kk, 0)]
            # next_quantiles_cost = next_quantiles_cost[...,:-int(trunc)] # -int(self.tail_cost*Nc)]
            # next_quantiles_cost = next_quantiles_cost[...,:-int(self.tail_cost*Nc)]
            target_q_value_c = cost[:, None, None] + self._cfgs.algo_cfgs.gamma * (1 - done[:,None, None]) * next_quantiles_cost.clamp_(-40., 0.)
    ...
    def _update_actor(  # pylint: disable=too-many-arguments
        ...
        if self._epoch > 200:
            explore_cost = self._logger.get_stats('Metrics/EpCost')[0]
            self._lagrange.update_delta_fly(explore_cost)
        #cost_test = self._logger.get_stats('Metrics/EpCost')[0]
        #self._lagrange.update_auto_truncation(cost_test/10., Jcm)
    ...
    def _loss_pi(
        ...
        with torch.no_grad():
            ...
            q_current_cost_raw = torch.stack(q_current_cost, -1)
            q_current_cost = -(q_current_cost_raw[:, :13]).mean(-1).mean(-1)
        

        lam = self._lagrange.lagrangian_multiplier.item()
        q_cost = self._actor_critic.cost_critic(obs, action)
        # q_cost = -torch.concatenate(q_cost, -1).mean(-1)
